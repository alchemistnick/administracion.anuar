...
modelo_seleccionado = st.sidebar.selectbox(
    "Seleccionar Modelo:", list(dict_modelos.keys())
)
...
with tab_dash:
    ...
    # Recopilar nóminas de todas las delegaciones desde Firestore
    nominas = []
    for d in delegaciones:
        id_del = d.get("id") or d.get("id_delegacion")
        if id_del:
            integrantes_escuela = obtener_integrantes_delegacion(id_del)
            nominas.extend(integrantes_escuela)

    # Mantener pagos desde API de manera temporal hasta migrar la tabla de pagos
...
with tab_ficha:
    ...
    if not delegaciones_ficha:
        ...
        opciones_escuelas = {
            f"[{d.get('id', d.get('id_delegacion'))}] {d.get('nombre_colegio', 'Sin Nombre')}": d
        ...
        id_del = escuela.get("id") or escuela.get("id_delegacion")
        ...
        with col_f2:
            st.markdown(
                f"**👤 Responsable:** {escuela.get('docente_apellido_nombre', '-')}"
            )
            st.markdown(
                f"**📧 Email Docente:** {escuela.get('docente_email', '-')}"
            )
            st.markdown(
                f"**📱 Teléfono Celular:** {escuela.get('docente_telefono', '-')}"
            )
        with col_f3:
            st.markdown(
                f"**📊 Cupos Solicitados:** {escuela.get('cupos_solicitados', '-')}"
            )
            st.markdown(
                f"**🔑 Clave de Acceso:** `{escuela.get('secret_hash', '-')}`"
            )
            st.markdown(
                f"**📌 Estado Documentación:** `{escuela.get('estado', 'REGISTRADO')}`"
            )
        ...
        if not bancas_escuela:
            st.warning(
                "⚠️ Esta institución aún no tiene bancas o países asignados."
            )
        else:
            for b in bancas_escuela:
                st.write(
                    f"- **{b.get('organo')}** — País: **{b.get('pais')}** (ID Asignación: `{b.get('id_asignacion')}`)"
                )

        st.markdown("---")
        st.markdown("### 👨‍🏫 Docentes Acompañantes Registrados")

        # --- LECTURA DE INTEGRANTES DE ESTA DELEGACIÓN DESDE FIREBASE ---
        ...
        if not docentes_escuela:
            ...
            for doc in docentes_escuela:
                st.write(
                    f"- **{doc.get('nombre', '')} {doc.get('apellido', '')}** (DNI: {doc.get('dni', doc.get('id'))}) — {doc.get('alergias_medicas', 'Sin especificaciones')}"
                )
        ...
        if not alumnos_escuela:
            ...
            cols_mostrar = [
                ...
                if c in df_alumnos.columns
            ]
            st.dataframe(df_alumnos[cols_mostrar], use_container_width=True)
            descargar_csv_para_excel(df_alumnos, f"nomina_{id_del}")

# 3. AUDITORÍA
with tab_auditoria:
    st.subheader("🔍 Auditoría de Documentación y Aprobación Final")
    delegaciones_aud = api_get(
        "GET_TODAS_DELEGACIONES", f"&id_modelo={id_modelo_actual}"
    )
    ...
    if not delegaciones_aud:
        ...
        opc_aud = {
            f"[{d.get('id_delegacion')}] {d.get('nombre_colegio')} (Estado: {d.get('estado')})": d
        ...
        nominas_todas_aud = api_get(
            "GET_TODAS_NOMINAS", f"&id_modelo={id_modelo_actual}"
        )
        registros_aud = [
            n
            for n in nominas_todas_aud
            if str(n.get("id_delegacion")).strip().upper()
            == str(id_del_aud).strip().upper()
        ]
        ...
        if not registros_aud:
            ...
            for idx, reg in enumerate(registros_aud):
                ...
                st.markdown(
                    ...
                    f" (DNI: {reg.get('dni')}) — *Rol/Banca:* {rol_txt}"
                )
            ...
            with col_btn1:
                if st.button(
                    ...
                    with st.spinner(
                        "Aprobando legajo y enviando correo..."
                    ):
                        try:
                            res_ap = requests.post(
                                API_URL, json=payload_aprobacion
                            ).json()
                ...
                with st.expander("❌ Rechazar Legajo con Observaciones"):
                    with st.form(key=f"form_rechazo_{id_del_aud}"):
                        motivo_rechazo = st.text_area(
                            "Indique el motivo del rechazo o corrección"
                            " necesaria:"
                        )
                        ...
                        if btn_enviar_rechazo:
                            if not motivo_rechazo.strip():
                                ...
                                with st.spinner(
                                    "Procesando rechazo y enviando correo..."
                                ):
                                    try:
                                        res_rec = requests.post(
                                            API_URL, json=payload_rechazo
                                        ).json()
                                        if res_rec.get("status") == "SUCCESS":
                                            st.warning(
                                                "¡Legajo rechazado y notificado"
                                                " a la escuela correctamente!"
                                            )
                                            st.rerun()
                                        else:
                                            st.error(
                                                "Error:"
                                                f" {res_rec.get('message')}"
                                            )
                                    except Exception as e:
                                        st.error(f"Error de conexión: {e}")
...
with tab_pagos:
    ...
    mapa_colegios = {
        d.get("id_delegacion"): d.get("nombre_colegio")
        for d in delegaciones_pagos
    }
    mapa_responsables = {
        d.get("id_delegacion"): d.get("docente_apellido_nombre")
        for d in delegaciones_pagos
    }
    ...
    with subtab1:
        if not pagos_pendientes:
            ...
            for p in pagos_pendientes:
                ...
                nombre_escuela = mapa_colegios.get(
                    id_del, "Institución Desconocida"
                )
                ...
                with st.container():
                    ...
                    with col_p1:
                        ...
                        st.write(
                            f"**🏛️ Institución:** {nombre_escuela} (`{id_del}`)"
                        )
                    ...
                    with col_p2:
                        ...
                        if url_comp:
                            st.markdown(
                                f"🔗 [Ver Comprobante en Drive]({url_comp})",
                                unsafe_allow_html=True,
                            )
    ...
    with subtab2:
        ...
        if pagos_todos:
            ...
            df_pagos["nombre_colegio"] = (
                df_pagos["id_delegacion"]
                .map(mapa_colegios)
                .fillna("Desconocido")
            )
            ...
            st.metric(
                "Total Recaudado (Pagos Aprobados)", f"${total_recaudado:,.2f}"
            )
...
with tab_paises:
    ...
    try:
        res_orgs = requests.get(
            f"{API_URL}?action=GET_ORGANOS_GENERAL"
        ).json()
        ...
        if tabla_organos:
            ...
            st.markdown(
                "### 🟢 Países y Bancas Disponibles / Sin Asignar (Solapa"
                f" Organos): `{len(sin_asignar_orgs)}`"
            )
            if not sin_asignar_orgs.empty:
                st.dataframe(
                    sin_asignar_orgs[
                        ["id_modelo", "organo_comite", "pais", "integrantes"]
                    ],
                    use_container_width=True,
                )
                descargar_csv_para_excel(
                    sin_asignar_orgs, "paises_disponibles_organos"
                )
            else:
                st.success(
                    "🎉 ¡No hay países libres en la solapa Organos! Todos tienen"
                    " asignación."
                )

            st.markdown("---")
            st.markdown(
                f"### 🔒 Países y Bancas Asignadas: `{len(asignados_orgs)}`"
            )
            if not asignados_orgs.empty:
                st.dataframe(asignados_orgs, use_container_width=True)
                descargar_csv_para_excel(
                    asignados_orgs, "paises_asignados_organos"
                )
            else:
                st.info(
                    "Aún no hay registros con ID de asignación en la solapa"
                    " Organos."
                )
        else:
            st.info(
                "No se encontraron datos en la solapa Organos de tu Google"
                " Sheet."
            )
    except Exception as e:
    ...
    if nominas_medicas:
        ...
        if not alerta_nominas:
            ...
            st.warning(
                ...
                " observaciones médicas o de contacto:"
            )
...
with tab_acred:
    ...
    try:
        res_acred = requests.get(
            f"{API_URL}?action=GET_ESTADISTICAS_ACREDITACION&id_modelo={id_modelo_actual}"
        ).json()
        if res_acred.get("status") == "SUCCESS":
            ...
            with col_a1:
                st.metric(
                    "Total en Nóminas", d_acred.get("total_nominados", 0)
                )
            with col_a2:
                st.metric(
                    "Total Acreditados", d_acred.get("total_acreditados", 0)
                )
            with col_a3:
                st.metric(
                    "Porcentaje de Acreditación",
                    f"{d_acred.get('porcentaje', 0)}%",
                )

            st.markdown("---")
            st.markdown(
                "### ❌ Participantes Pendientes de Acreditación (Ausentes en"
                " este Modelo)"
            )
            ...
            if no_acred:
                ...
                st.dataframe(
                    df_no[
                        ["id_delegacion", "nombre", "apellido", "dni", "rol_mnu"]
                    ],
                    use_container_width=True,
                )
                descargar_csv_para_excel(
                    df_no, f"pendientes_acreditacion_{id_modelo_actual}"
                )
            else:
                st.success(
                    "🎉 ¡Todos los participantes de este modelo se encuentran"
                    " acreditados!"
                )
    except Exception as e:
