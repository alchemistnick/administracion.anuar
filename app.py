...
def actualizar_estado_delegacion(email_delegacion, nuevo_estado):
    try:
        db.collection("delegaciones").document(email_delegacion).update(
            {"estado": nuevo_estado}
        )
        return True
    except Exception as e:
...
def actualizar_estado_pago(pago_id, nuevo_estado):
    try:
        db.collection("pagos").document(pago_id).update(
...
tab1, tab2, tab3 = st.tabs(
    [
        "🏛️ Delegaciones e Instituciones",
        "💳 Gestión de Pagos",
...
with tab1:
    ...
    if not delegaciones:
        ...
        for d in delegaciones:
            with st.expander(
                f"🏫 {d.get('nombre_colegio', 'Colegio')} — Docente:"
                f" {d.get('docente_apellido_nombre', 'N/A')}"
                f" ({d.get('id_delegacion')})"
            ):
                col1, col2, col3 = st.columns(3)
                with col1:
                    st.write(
                        f"**Email Institucional:** {d.get('email_institucional')}"
                    )
                ...
                with col2:
                    st.write(
                        ...
                        f" {d.get('docente_apellido_nombre')}"
                    )
                    st.write(f"**Email Docente:** {d.get('docente_email')}")
                    st.write(f"**Móvil Docente:** {d.get('docente_telefono')}")
                with col3:
                    st.write(
                        f"**Cupos Solicitados:** {d.get('cupos_solicitados')}"
                    )
                    st.write(
                        f"**Acompañantes:** {d.get('docentes_acompanantes')}"
                    )
                ...
                with col_btn1:
                    if st.button(
                        "✅ Aprobar Legajo",
                        key=f"aprobar_{d.get('id_delegacion')}",
                    ):
                        if actualizar_estado_delegacion(
                            d.get("id_delegacion"), "APROBADO"
                        ):
                            notificar_accion_script(
                                "APROBAR_LEGAJO_ESCUELA",
                ...
                with col_btn2:
                    motivo_rechazo = st.text_input(
                        ...
                        key=f"mot_{d.get('id_delegacion')}",
                    )
                    if st.button(
                        "⚠️ Rechazar / Observar",
                        key=f"rech_{d.get('id_delegacion')}",
                    ):
                        if actualizar_estado_delegacion(
                            d.get("id_delegacion"), "OBSERVADO"
                        ):
                            notificar_accion_script(
                                "RECHAZAR_LEGAJO_ESCUELA",
                                {
                                    ...
                                    "motivo": (
                                        motivo_rechazo
                                        or "Revisar documentación faltante."
                                    ),
                                },
                            )
                            st.warning(
                                "Se ha marcado como observado y notificado."
                            )
                            st.rerun()
        ...
        for p in pagos:
            with st.container():
                ...
                with col_p3:
                    ...
                    st.markdown(
                        f"[📄 Ver Comprobante en Drive]({drive_url})",
                ...
                with col_p4:
                    nuevo_est = st.selectbox(
                        ...
                        index=[
                            ...
                            "APROBADO",
                            "RECHAZADO",
                        ].index(p.get("estado_pago", "PENDIENTE")),
                    )
                    if st.button(
                        "💾 Actualizar Pago", key=f"btn_pago_{p.get('id')}"
                    ):
...
with tab3:
    ...
    if not delegaciones:
        ...
        delegacion_sel = st.selectbox(
            "Seleccionar Institución para ver Estudiantes:", emails_del
        )
        ...
        if delegacion_sel:
            ...
            try:
                ...
                if not integrantes:
                    st.info(
                        "Esta institución aún no ha cargado estudiantes en su"
                        " nómina."
                    )
                else:
                    for est in integrantes:
                        with st.expander(
                            f"👤 {est.get('nombre')} {est.get('apellido')} (DNI:"
                            f" {est.get('dni')})"
                        ):
                            col_e1, col_e2 = st.columns(2)
                            with col_e1:
                                st.write(
                                    f"**Alergias / Condiciones:**"
                                    f" {est.get('alergias_medicas', 'Ninguna')}"
                                )
                                st.write(
                                    f"**Asignación:**"
                                    f" {est.get('id_asignacion', 'Sin asignar')}"
                                )
                                st.write(
                                    f"**Observaciones:**"
                                    f" {est.get('comentarios', 'Ninguna')}"
                                )
                            with col_e2:
